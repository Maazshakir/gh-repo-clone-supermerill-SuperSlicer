...
void PrintObject::_offsetHoles(float hole_delta, LayerRegion *layerm) {
    std::cout << "offset holes by " << unscale(hole_delta) << "\n";
    if (hole_delta != 0.f) {
        ExPolygons& polys = to_expolygons(std::move(layerm->slices.surfaces));
        ExPolygons new_polys;
        for (ExPolygon ex_poly : polys) {
            std::cout << "poly " << unscale(unscale(ex_poly.area())) << "\n";
            ExPolygon new_ex_poly(ex_poly);
            new_ex_poly.holes.clear();
            for (Polygon hole : ex_poly.holes) {
                std::cout << "hole " << unscale(unscale(hole.area())) << "\n";
                std::cout << "points: " << hole.points.size()<<", convex="<<hole.convex_points(PI).size() << "\n";
                //check if convex to reduce it
                // check whether first point forms a convex angle
                bool ok = true;
                std::cout << "first : " << hole.points.front().ccw_angle(hole.points.back(), *(hole.points.begin() + 1)) << "\n";
                ...
                if (ok)
                    for (Points::const_iterator p = hole.points.begin() + 1; p != hole.points.end() - 1; ++p) {
                        std::cout << "next : " << p->ccw_angle(*(p - 1), *(p + 1)) << "\n";
                        ok = (p->ccw_angle(*(p - 1), *(p + 1)) <= PI + 0.001);
                        if (!ok) {
                            break;
                            std::cout << "break! : " << p->ccw_angle(*(p - 1), *(p + 1)) << "\n";
                        }
                    }

                // check whether last point forms a convex angle
                std::cout << "last : " << hole.points.back().ccw_angle(*(hole.points.end() - 2), hole.points.front()) << "\n";
                ok &= (hole.points.back().ccw_angle(*(hole.points.end() - 2), hole.points.front()) <= PI + 0.001);

                if (ok) {
                    for (Polygon newHole : offset(hole, hole_delta)) {
                        std::cout << "grow " << unscale(unscale(newHole.area())) << "\n";
                ...
                std::cout << "new poly has nbholes = " << new_ex_poly.holes.size() << "\n";
            }
            for (Polygon hole : new_ex_poly.holes) {
                std::cout << "new poly hole = " << unscale(unscale(hole.area())) << "\n";
            }
            std::cout << "new poly " << unscale(unscale(new_ex_poly.area())) << "\n";
