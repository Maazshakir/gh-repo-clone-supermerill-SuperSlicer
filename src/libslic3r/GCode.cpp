...
std::string GCode::extrude_loop_vase(const ExtrusionLoop &original_loop, const std::string &description, double speed, std::unique_ptr<EdgeGrid::Grid> *lower_layer_edge_grid)
{
    ...
    std::cout << "init z = " << current_z << " , h = " << current_height <<",lengh=" <<length<<", height = "<< this->m_layer->height<<"\n";
    enum Step {
        INCR = 0,
        FLAT = 1
    };
    ...
    for (int step = 0; step < 2; step++) {
        ...
        std::cout << "step z = " << current_z << " , h = " << current_height << ", z_per_length=" << z_per_length << ", height_per_length=" << height_per_length << "\n";
        ...
        for (ExtrusionPaths::iterator path = paths.begin(); path != paths.end(); ++path) {
            ...
            //go to first Z
            //gcode += m_writer.travel_to_z(current_z, description);
            ...
            if (m_writer.extrusion_axis().empty()) e_per_mm_per_height = 0;
            {
                ...
                for (const Line &line : path->polyline.lines()) {
                    ...
                    const int nb_sections = int(line_length / EXTRUDER_CONFIG(nozzle_diameter));
                    const coordf_t height_increment = height_per_length * line_length / nb_sections;
                    std::cout << "line nb_sections=" << nb_sections << ", height_increment="<< height_increment<<" :: ";
                    ...
                    std::cout << "end line("<< (100* current_pos_in_length / length) <<"%) z = " << current_z << " , h = " << current_height << "\n";
                }
            }
            gcode += this->_after_extrude(*path);
        }
        std::cout << "END step z = " << current_z << " , h = " << current_height << "\n";
    }

    // reset acceleration
    gcode += m_writer.set_acceleration((unsigned int)(m_config.default_acceleration.value + 0.5));
    std::cout << "gcode extruded =  \n" << gcode << "\n";
